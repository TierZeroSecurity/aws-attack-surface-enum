#!/usr/bin/env bash
# aws-attack-surface-enum-extended.sh
# Extended AWS external attack surface enumeration with traffic flow analysis
# Enhanced for: NAT gateways, Transit Gateway, route tables, VPC endpoints, WAF, SCPs
...
echo "=============================================="
echo "  AWS Attack Surface Enumeration (Extended)"
echo "  Coverage: 45+ Services + Network Analysis"
echo "=============================================="
echo ""
...
echo ""
info "Starting enhanced attack surface enumeration..."
echo ""
...
CSV_HEADERS=(
  ...
  ["nat-gateways.csv"]="Region,NatGatewayId,State,SubnetId,VpcId,PublicIP,PrivateIP,Name"
  ["internet-gateways.csv"]="Region,InternetGatewayId,VpcId,State,Name"
  ["transit-gateway-attachments.csv"]="Region,AttachmentId,TransitGatewayId,ResourceType,ResourceId,State,Name"
  ["transit-gateways.csv"]="Region,TransitGatewayId,State,OwnerId,Description,Name"
  ["route-tables.csv"]="Region,RouteTableId,VpcId,DestinationCidr,Target,State,Name"
  ["vpc-endpoints.csv"]="Region,VpcEndpointId,Type,ServiceName,State,VpcId,Subnets,Name"
  ["vpn-connections.csv"]="Region,VpnConnectionId,State,Type,CustomerGatewayId,VpnGatewayId,Name"
  ["direct-connect.csv"]="Region,ConnectionId,ConnectionName,ConnectionState,Location,Bandwidth"
  ["vpc-peering.csv"]="Region,VpcPeeringConnectionId,RequesterVpcId,AccepterVpcId,Status,Name"
  # NEW: Security controls
  ["waf-associations.csv"]="Region,WebACLName,WebACLArn,Scope,AssociatedResources"
  ["shield-protections.csv"]="Region,ProtectionId,ResourceArn,Name"
  ["network-acls.csv"]="Region,NetworkAclId,VpcId,IsDefault,RuleNumber,Protocol,Action,CidrBlock"
  ["vpc-flow-logs.csv"]="Region,FlowLogId,ResourceType,ResourceId,TrafficType,LogDestination,Status"
  ["cloudguard-instances.csv"]="Region,InstanceId,InstanceType,PrivateIP,PublicIP,ImageId,Name"
  # NEW: Enhanced load balancer details
  ["lb-detailed.csv"]="Region,LoadBalancerName,LoadBalancerArn,Listeners,WAF,SecurityGroups,Scheme"
  ["lb-target-groups.csv"]="Region,TargetGroupName,TargetGroupArn,Protocol,Port,HealthCheckPath,TargetCount"
  # NEW: API Gateway detailed
  ["api-gateway-detailed.csv"]="Region,ApiName,ApiId,Endpoint,Stage,AuthType,ResourcePolicy,CustomDomain"
  ["lambda-functions.csv"]="Region,FunctionName,FunctionArn,Runtime,VpcId,PublicAccess,LastModified"
)
...
# ============================================================================
# ORIGINAL SCANNERS (from base script)
# ============================================================================
...
# ============================================================================
# NEW SCANNERS: Network Infrastructure
# ============================================================================

scan_nat_gateways() {
  local region=$1
  if data=$(aws_call aws ec2 describe-nat-gateways --region "$region" --output json 2>/dev/null); then
    printf '%s\n' "$data" > "$OUTDIR/raw/nat-gateways-${region}.json"
    echo "$data" | jq -r --arg r "$region" '
      .NatGateways[]? |
      [$r, .NatGatewayId, .State, .SubnetId, .VpcId,
       ((.NatGatewayAddresses[]?.PublicIp // []) | if type=="array" then join(";") else . end),
       ((.NatGatewayAddresses[]?.PrivateIp // []) | if type=="array" then join(";") else . end),
       ((.Tags[]? | select(.Key=="Name") | .Value) // "")] | @csv
    ' 2>/dev/null | while IFS= read -r row; do
      [ -n "$row" ] && write_tmp_csv "nat-gateways.csv" "$row" "$region"
    done
  fi
}
...
scan_internet_gateways() {
  local region=$1
  if data=$(aws_call aws ec2 describe-internet-gateways --region "$region" --output json 2>/dev/null); then
    printf '%s\n' "$data" > "$OUTDIR/raw/igw-${region}.json"
    echo "$data" | jq -r --arg r "$region" '
      .InternetGateways[]? |
      [$r, .InternetGatewayId,
       ((.Attachments[]?.VpcId // []) | if type=="array" then join(";") else . end),
       ((.Attachments[]?.State // []) | if type=="array" then join(";") else . end),
       ((.Tags[]? | select(.Key=="Name") | .Value) // "")] | @csv
    ' 2>/dev/null | while IFS= read -r row; do
      [ -n "$row" ] && write_tmp_csv "internet-gateways.csv" "$row" "$region"
    done
  fi
}
...
scan_transit_gateways() {
  local region=$1
  if data=$(aws_call aws ec2 describe-transit-gateways --region "$region" --output json 2>/dev/null); then
    printf '%s\n' "$data" > "$OUTDIR/raw/tgw-${region}.json"
    echo "$data" | jq -r --arg r "$region" '
      .TransitGateways[]? |
      [$r, .TransitGatewayId, .State, .OwnerId,
       (.Description // ""),
       ((.Tags[]? | select(.Key=="Name") | .Value) // "")] | @csv
    ' 2>/dev/null | while IFS= read -r row; do
      [ -n "$row" ] && write_tmp_csv "transit-gateways.csv" "$row" "$region"
    done
  fi
}

scan_transit_gateway_attachments() {
  local region=$1
  if data=$(aws_call aws ec2 describe-transit-gateway-attachments --region "$region" --output json 2>/dev/null); then
    printf '%s\n' "$data" > "$OUTDIR/raw/tgw-attachments-${region}.json"
    echo "$data" | jq -r --arg r "$region" '
      .TransitGatewayAttachments[]? |
      [$r, .TransitGatewayAttachmentId, .TransitGatewayId,
       .ResourceType, .ResourceId, .State,
       ((.Tags[]? | select(.Key=="Name") | .Value) // "")] | @csv
    ' 2>/dev/null | while IFS= read -r row; do
      [ -n "$row" ] && write_tmp_csv "transit-gateway-attachments.csv" "$row" "$region"
    done
  fi
}
...
scan_route_tables() {
  local region=$1
  if data=$(aws_call aws ec2 describe-route-tables --region "$region" --output json 2>/dev/null); then
    printf '%s\n' "$data" > "$OUTDIR/raw/route-tables-${region}.json"
    echo "$data" | jq -r --arg r "$region" '
      .RouteTables[]? as $rt |
      $rt.Routes[]? |
      [$r, $rt.RouteTableId, $rt.VpcId,
       (.DestinationCidrBlock // .DestinationIpv6CidrBlock // ""),
       (.GatewayId // .NatGatewayId // .TransitGatewayId // .VpcPeeringConnectionId // .NetworkInterfaceId // "local"),
       .State,
       (($rt.Tags[]? | select(.Key=="Name") | .Value) // "")] | @csv
    ' 2>/dev/null | while IFS= read -r row; do
      [ -n "$row" ] && write_tmp_csv "route-tables.csv" "$row" "$region"
    done
  fi
}
...
scan_vpc_endpoints() {
  local region=$1
  if data=$(aws_call aws ec2 describe-vpc-endpoints --region "$region" --output json 2>/dev/null); then
    printf '%s\n' "$data" > "$OUTDIR/raw/vpc-endpoints-${region}.json"
    echo "$data" | jq -r --arg r "$region" '
      .VpcEndpoints[]? |
      [$r, .VpcEndpointId, .VpcEndpointType, .ServiceName,
       .State, .VpcId,
       ((.SubnetIds // []) | join(";")),
       ((.Tags[]? | select(.Key=="Name") | .Value) // "")] | @csv
    ' 2>/dev/null | while IFS= read -r row; do
      [ -n "$row" ] && write_tmp_csv "vpc-endpoints.csv" "$row" "$region"
    done
  fi
}
...
scan_vpn_connections() {
  local region=$1
  if data=$(aws_call aws ec2 describe-vpn-connections --region "$region" --output json 2>/dev/null); then
    printf '%s\n' "$data" > "$OUTDIR/raw/vpn-${region}.json"
    echo "$data" | jq -r --arg r "$region" '
      .VpnConnections[]? |
      [$r, .VpnConnectionId, .State, .Type,
       .CustomerGatewayId, (.VpnGatewayId // .TransitGatewayId // ""),
       ((.Tags[]? | select(.Key=="Name") | .Value) // "")] | @csv
    ' 2>/dev/null | while IFS= read -r row; do
      [ -n "$row" ] && write_tmp_csv "vpn-connections.csv" "$row" "$region"
    done
  fi
}
...
scan_direct_connect() {
  local region=$1
  if data=$(aws_call aws directconnect describe-connections --region "$region" --output json 2>/dev/null); then
    printf '%s\n' "$data" > "$OUTDIR/raw/dx-${region}.json"
    echo "$data" | jq -r --arg r "$region" '
      .connections[]? |
      [$r, .connectionId, .connectionName, .connectionState,
       .location, .bandwidth] | @csv
    ' 2>/dev/null | while IFS= read -r row; do
      [ -n "$row" ] && write_tmp_csv "direct-connect.csv" "$row" "$region"
    done
  fi
}
...
scan_vpc_peering() {
  local region=$1
  if data=$(aws_call aws ec2 describe-vpc-peering-connections --region "$region" --output json 2>/dev/null); then
    printf '%s\n' "$data" > "$OUTDIR/raw/vpc-peering-${region}.json"
    echo "$data" | jq -r --arg r "$region" '
      .VpcPeeringConnections[]? |
      [$r, .VpcPeeringConnectionId,
       .RequesterVpcInfo.VpcId, .AccepterVpcInfo.VpcId,
       .Status.Code,
       ((.Tags[]? | select(.Key=="Name") | .Value) // "")] | @csv
    ' 2>/dev/null | while IFS= read -r row; do
      [ -n "$row" ] && write_tmp_csv "vpc-peering.csv" "$row" "$region"
    done
  fi
}

# ============================================================================
# NEW SCANNERS: Security Controls
# ============================================================================

scan_waf_associations() {
  local region=$1
  # WAFv2 (regional)
  if data=$(aws_call aws wafv2 list-web-acls --scope REGIONAL --region "$region" --output json 2>/dev/null); then
    printf '%s\n' "$data" > "$OUTDIR/raw/waf-regional-${region}.json"
    echo "$data" | jq -r '.WebACLs[]?.ARN' 2>/dev/null | while IFS= read -r arn; do
      [ -z "$arn" ] && continue
      resources=$(aws_call aws wafv2 list-resources-for-web-acl --web-acl-arn "$arn" --region "$region" --output json 2>/dev/null | jq -r '.ResourceArns[]?' 2>/dev/null | tr '\n' ';' | sed 's/;$//' || echo "")
      name=$(echo "$data" | jq -r --arg arn "$arn" '.WebACLs[] | select(.ARN==$arn) | .Name')
      row=$(jq -nr --arg r "$region" --arg name "$name" --arg arn "$arn" --arg res "$resources" '[$r,$name,$arn,"REGIONAL",$res] | @csv')
      write_tmp_csv "waf-associations.csv" "$row" "$region"
    done
  fi
}

scan_shield_protections() {
  local region=$1
  if data=$(aws_call aws shield list-protections --region "$region" --output json 2>/dev/null); then
    printf '%s\n' "$data" > "$OUTDIR/raw/shield-${region}.json"
    echo "$data" | jq -r --arg r "$region" '
      .Protections[]? |
      [$r, .Id, .ResourceArn, .Name] | @csv
    ' 2>/dev/null | while IFS= read -r row; do
      [ -n "$row" ] && write_tmp_csv "shield-protections.csv" "$row" "$region"
    done
  fi
}
...
scan_network_acls() {
  local region=$1
  if data=$(aws_call aws ec2 describe-network-acls --region "$region" --output json 2>/dev/null); then
    printf '%s\n' "$data" > "$OUTDIR/raw/nacl-${region}.json"
    echo "$data" | jq -r --arg r "$region" '
      .NetworkAcls[]? as $nacl |
      $nacl.Entries[]? |
      [$r, $nacl.NetworkAclId, $nacl.VpcId, $nacl.IsDefault,
       .RuleNumber, .Protocol, .RuleAction,
       (.CidrBlock // .Ipv6CidrBlock // "")] | @csv
    ' 2>/dev/null | while IFS= read -r row; do
      [ -n "$row" ] && write_tmp_csv "network-acls.csv" "$row" "$region"
    done
  fi
}
...
scan_vpc_flow_logs() {
  local region=$1
  if data=$(aws_call aws ec2 describe-flow-logs --region "$region" --output json 2>/dev/null); then
    printf '%s\n' "$data" > "$OUTDIR/raw/vpc-flow-logs-${region}.json"
    echo "$data" | jq -r --arg r "$region" '
      .FlowLogs[]? |
      [$r, .FlowLogId, .ResourceType, .ResourceId,
       .TrafficType, .LogDestination, .FlowLogStatus] | @csv
    ' 2>/dev/null | while IFS= read -r row; do
      [ -n "$row" ] && write_tmp_csv "vpc-flow-logs.csv" "$row" "$region"
    done
  fi
}

scan_cloudguard_instances() {
  local region=$1
  if data=$(aws_call aws ec2 describe-instances --region "$region" --filters "Name=instance-state-name,Values=running" --output json 2>/dev/null); then
    printf '%s\n' "$data" > "$OUTDIR/raw/ec2-instances-${region}.json"
    echo "$data" | jq -r --arg r "$region" '
      .Reservations[]?.Instances[]? |
      select(
        (.ImageId | test("checkpoint|cloudguard"; "i")) or
        (.Tags[]? | .Key == "Product" and (.Value | test("checkpoint|cloudguard"; "i")))
      ) |
      [$r, .InstanceId, .InstanceType, .PrivateIpAddress,
       (.PublicIpAddress // ""), .ImageId,
       ((.Tags[]? | select(.Key=="Name") | .Value) // "")] | @csv
    ' 2>/dev/null | while IFS= read -r row; do
      [ -n "$row" ] && write_tmp_csv "cloudguard-instances.csv" "$row" "$region"
    done
  fi
}

# ============================================================================
# NEW SCANNERS: Enhanced Details
# ============================================================================

scan_load_balancers_detailed() {
  local region=$1
  if lbs=$(aws_call aws elbv2 describe-load-balancers --region "$region" --output json 2>/dev/null); then
    echo "$lbs" | jq -r '.LoadBalancers[]? | .LoadBalancerArn' 2>/dev/null | while IFS= read -r lb_arn; do
      [ -z "$lb_arn" ] && continue

      listeners=$(aws_call aws elbv2 describe-listeners --load-balancer-arn "$lb_arn" --region "$region" --output json 2>/dev/null | jq -r '.Listeners[]? | "\(.Port)/\(.Protocol)"' 2>/dev/null | tr '\n' ';' | sed 's/;$//' || echo "")

      waf=$(aws_call aws wafv2 get-web-acl-for-resource --resource-arn "$lb_arn" --region "$region" --scope REGIONAL --output json 2>/dev/null | jq -r '.WebACL.Name // "None"' 2>/dev/null || echo "None")

      lb_name=$(echo "$lbs" | jq -r --arg arn "$lb_arn" '.LoadBalancers[] | select(.LoadBalancerArn==$arn) | .LoadBalancerName')
      sgs=$(echo "$lbs" | jq -r --arg arn "$lb_arn" '.LoadBalancers[] | select(.LoadBalancerArn==$arn) | .SecurityGroups[]?' 2>/dev/null | tr '\n' ';' | sed 's/;$//' || echo "")
      scheme=$(echo "$lbs" | jq -r --arg arn "$lb_arn" '.LoadBalancers[] | select(.LoadBalancerArn==$arn) | .Scheme')

      row=$(jq -nr --arg r "$region" --arg name "$lb_name" --arg arn "$lb_arn" --arg listeners "$listeners" --arg waf "$waf" --arg sgs "$sgs" --arg scheme "$scheme" '[$r,$name,$arn,$listeners,$waf,$sgs,$scheme] | @csv')
      write_tmp_csv "lb-detailed.csv" "$row" "$region"
    done
  fi
}

scan_lb_target_groups() {
  local region=$1
  if tgs=$(aws_call aws elbv2 describe-target-groups --region "$region" --output json 2>/dev/null); then
    echo "$tgs" | jq -r '.TargetGroups[]? | .TargetGroupArn' 2>/dev/null | while IFS= read -r tg_arn; do
      [ -z "$tg_arn" ] && continue

      tg_name=$(echo "$tgs" | jq -r --arg arn "$tg_arn" '.TargetGroups[] | select(.TargetGroupArn==$arn) | .TargetGroupName')
      protocol=$(echo "$tgs" | jq -r --arg arn "$tg_arn" '.TargetGroups[] | select(.TargetGroupArn==$arn) | .Protocol')
      port=$(echo "$tgs" | jq -r --arg arn "$tg_arn" '.TargetGroups[] | select(.TargetGroupArn==$arn) | .Port')
      health_path=$(echo "$tgs" | jq -r --arg arn "$tg_arn" '.TargetGroups[] | select(.TargetGroupArn==$arn) | .HealthCheckPath // "N/A"')

      target_health=$(aws_call aws elbv2 describe-target-health --target-group-arn "$tg_arn" --region "$region" --output json 2>/dev/null | jq -r '.TargetHealthDescriptions | length' || echo "0")

      row=$(jq -nr --arg r "$region" --arg name "$tg_name" --arg arn "$tg_arn" --arg proto "$protocol" --arg port "$port" --arg hp "$health_path" --arg tc "$target_health" '[$r,$name,$arn,$proto,$port,$hp,$tc] | @csv')
      write_tmp_csv "lb-target-groups.csv" "$row" "$region"
    done
  fi
}

scan_api_gateway_detailed() {
  local region=$1
  if rest=$(aws_call aws apigateway get-rest-apis --region "$region" --output json 2>/dev/null); then
    echo "$rest" | jq -r '.items[]? | [.name, .id] | @tsv' 2>/dev/null | while IFS=$'\t' read -r name id; do
      [ -z "$id" ] && continue
      ...
      policy=$(aws_call aws apigateway get-rest-api --rest-api-id "$id" --region "$region" --output json 2>/dev/null | jq -r '.policy // "None"' | head -c 50 || echo "None")

      custom_domains=$(aws_call aws apigateway get-domain-names --region "$region" --output json 2>/dev/null | jq -r --arg id "$id" '.items[]? | select(.regionalDomainName | contains($id)) | .domainName' 2>/dev/null | tr '\n' ';' | sed 's/;$//' || echo "")

      row=$(jq -nr --arg r "$region" --arg n "$name" --arg id "$id" --arg ep "$endpoint" --arg st "$stages" --arg auth "API_KEY" --arg pol "$policy" --arg cd "$custom_domains" '[$r,$n,$id,$ep,$st,$auth,$pol,$cd] | @csv')
      write_tmp_csv "api-gateway-detailed.csv" "$row" "$region"
    done
  fi
}
...
scan_lambda_functions() {
  ...
  while :; do
    ...
    echo "$out" | jq -r --arg r "$region" '
      .Functions[]? |
      [$r, .FunctionName, .FunctionArn, .Runtime,
       (.VpcConfig.VpcId // "No-VPC"),
       "Check-Policy",
       .LastModified] | @csv
    ' 2>/dev/null | while IFS= read -r row; do
      [ -n "$row" ] && write_tmp_csv "lambda-functions.csv" "$row" "$region"
    done
...
export -f scan_nat_gateways scan_internet_gateways scan_transit_gateways scan_transit_gateway_attachments
export -f scan_route_tables scan_vpc_endpoints scan_vpn_connections scan_direct_connect scan_vpc_peering
export -f scan_waf_associations scan_shield_protections scan_network_acls scan_vpc_flow_logs scan_cloudguard_instances
export -f scan_load_balancers_detailed scan_lb_target_groups scan_api_gateway_detailed scan_lambda_functions
export OUTDIR TMPDIR ACCOUNT_ID AWS_TIMEOUT AWS_CALL_MAX_ATTEMPTS

# ============================================================================
# Region scanning
# ============================================================================
SERVICES_PER_REGION=47
...
for region in "${REGIONS[@]}"; do
  ...
  jobs=(
    ...
    "scan_nat_gateways $region"
    "scan_internet_gateways $region"
    "scan_transit_gateways $region"
    "scan_transit_gateway_attachments $region"
    "scan_route_tables $region"
    "scan_vpc_endpoints $region"
    "scan_vpn_connections $region"
    "scan_direct_connect $region"
    "scan_vpc_peering $region"
    # Security controls
    "scan_waf_associations $region"
    "scan_shield_protections $region"
    "scan_network_acls $region"
    "scan_vpc_flow_logs $region"
    "scan_cloudguard_instances $region"
    # Enhanced details
    "scan_load_balancers_detailed $region"
    "scan_lb_target_groups $region"
    "scan_api_gateway_detailed $region"
...
# Global Accelerator
printf "  [G4/6] %-35s" "Global Accelerator..."
...
printf "  [G5/6] %-35s" "WAF (CloudFront)..."
if waf_global=$(aws_call aws wafv2 list-web-acls --scope CLOUDFRONT --region us-east-1 --output json 2>/dev/null); then
  printf '%s\n' "$waf_global" > "$OUTDIR/raw/waf-cloudfront.json"
  echo "$waf_global" | jq -r '.WebACLs[]?.ARN' 2>/dev/null | while IFS= read -r arn; do
    [ -z "$arn" ] && continue
    resources=$(aws_call aws wafv2 list-resources-for-web-acl --web-acl-arn "$arn" --region us-east-1 --output json 2>/dev/null | jq -r '.ResourceArns[]?' 2>/dev/null | tr '\n' ';' | sed 's/;$//' || echo "")
    name=$(echo "$waf_global" | jq -r --arg arn "$arn" '.WebACLs[] | select(.ARN==$arn) | .Name')
    row=$(jq -nr --arg name "$name" --arg arn "$arn" --arg res "$resources" '["us-east-1",$name,$arn,"CLOUDFRONT",$res] | @csv')
    write_tmp_csv "waf-associations.csv" "$row" "global"
  done
fi
merge_tmp_csv "waf-associations.csv"
echo " ✓"

# S3 Account Public Access Block
printf "  [G6/6] %-35s" "S3 Account Block Settings..."
if pab=$(aws_call aws s3control get-public-access-block --account-id "$ACCOUNT_ID" --output json 2>/dev/null); then
  printf '%s\n' "$pab" > "$OUTDIR/raw/s3-account-public-access-block.json"

  block_acls=$(echo "$pab" | jq -r '.PublicAccessBlockConfiguration.BlockPublicAcls')
  block_policy=$(echo "$pab" | jq -r '.PublicAccessBlockConfiguration.BlockPublicPolicy')
  ignore_acls=$(echo "$pab" | jq -r '.PublicAccessBlockConfiguration.IgnorePublicAcls')
  restrict=$(echo "$pab" | jq -r '.PublicAccessBlockConfiguration.RestrictPublicBuckets')

  echo "AccountID,BlockPublicAcls,BlockPublicPolicy,IgnorePublicAcls,RestrictPublicBuckets" > "$OUTDIR/s3-account-public-access-block.csv"
  echo "$ACCOUNT_ID,$block_acls,$block_policy,$ignore_acls,$restrict" >> "$OUTDIR/s3-account-public-access-block.csv"
  echo " ✓"
else
  echo " (Not configured)"
fi
...
info "Generating enhanced summary..."

{
  echo "╔════════════════════════════════════════════════════════╗"
  echo "║   AWS Attack Surface Inventory (Extended)              ║"
  echo "║   Coverage: 45+ Services + Network Flow Analysis       ║"
  echo "╚════════════════════════════════════════════════════════╝"
  echo ""
  echo "Scan Date:       $(date)"
  ...
  echo ""
  echo "═══════════════════════════════════════════════════════"
  echo "Resource Counts by Category"
  echo "═══════════════════════════════════════════════════════"
  echo ""
  ...
  for f in nat-gateways internet-gateways transit-gateways transit-gateway-attachments route-tables vpc-endpoints vpn-connections direct-connect vpc-peering; do
    if [ -f "$OUTDIR/${f}.csv" ]; then
      count=$(($(wc -l < "$OUTDIR/${f}.csv" 2>/dev/null || echo "1") - 1))
      [ "$count" -lt 0 ] && count=0
      total_resources=$((total_resources + count))
      [ "$count" -gt 0 ] && printf "  %-45s : %5d\n" "$f" "$count"
    fi
  done

  echo ""
  echo "SECURITY CONTROLS:"
  for f in waf-associations shield-protections network-acls vpc-flow-logs cloudguard-instances; do
    if [ -f "$OUTDIR/${f}.csv" ]; then
      count=$(($(wc -l < "$OUTDIR/${f}.csv" 2>/dev/null || echo "1") - 1))
      [ "$count" -lt 0 ] && count=0
      total_resources=$((total_resources + count))
  ...
  for f in public-enis elastic-ips loadbalancers api-gateway lambda-urls; do
    if [ -f "$OUTDIR/${f}.csv" ]; then
      count=$(($(wc -l < "$OUTDIR/${f}.csv" 2>/dev/null || echo "1") - 1))
      [ "$count" -lt 0 ] && count=0
      total_resources=$((total_resources + count))
  ...
  echo ""
  echo "DATABASES:"
  for f in rds redshift opensearch msk elasticache docdb neptune memorydb; do
    if [ -f "$OUTDIR/${f}.csv" ]; then
      count=$(($(wc -l < "$OUTDIR/${f}.csv" 2>/dev/null || echo "1") - 1))
      [ "$count" -lt 0 ] && count=0
      total_resources=$((total_resources + count))
  ...
  echo "═══════════════════════════════════════════════════════"
  echo ""
  echo "🔍 Traffic Flow & Security Analysis"
  echo "═══════════════════════════════════════════════════════"
  echo ""

  # Count key security metrics
  public_ips=0
  tgw_attachments=0
  igw_count=0
  nat_count=0
  ...
  if [ -f "$OUTDIR/public-enis.csv" ]; then
    public_ips=$(($(wc -l < "$OUTDIR/public-enis.csv") - 1))
    [ "$public_ips" -lt 0 ] && public_ips=0
  fi

  if [ -f "$OUTDIR/transit-gateway-attachments.csv" ]; then
    tgw_attachments=$(($(wc -l < "$OUTDIR/transit-gateway-attachments.csv") - 1))
    [ "$tgw_attachments" -lt 0 ] && tgw_attachments=0
  fi

  if [ -f "$OUTDIR/internet-gateways.csv" ]; then
    igw_count=$(($(wc -l < "$OUTDIR/internet-gateways.csv") - 1))
    [ "$igw_count" -lt 0 ] && igw_count=0
  fi

  if [ -f "$OUTDIR/nat-gateways.csv" ]; then
    nat_count=$(($(wc -l < "$OUTDIR/nat-gateways.csv") - 1))
    [ "$nat_count" -lt 0 ] && nat_count=0
  fi

  if [ -f "$OUTDIR/lb-detailed.csv" ]; then
    waf_protected=$(awk -F',' 'NR>1 && $5 !~ /None/ {count++} END {print count+0}' "$OUTDIR/lb-detailed.csv" 2>/dev/null || echo "0")
  fi

  if [ -f "$OUTDIR/sg-open.csv" ]; then
    open_sgs=$(($(wc -l < "$OUTDIR/sg-open.csv") - 1))
    [ "$open_sgs" -lt 0 ] && open_sgs=0
  fi

  echo "Network Egress Points:"
  echo "  • $nat_count NAT Gateways (outbound internet access)"
  echo "  • $igw_count Internet Gateways (direct internet routes)"
  echo "  • $tgw_attachments Transit Gateway Attachments (centralized routing)"
  echo ""
  echo "Public Exposure:"
  echo "  • $public_ips Public IP addresses (all services)"
  echo "  • $open_sgs Security group rules with 0.0.0.0/0 or ::/0"
  echo ""
  echo "Protection Status:"
  echo "  • $waf_protected Load balancers with WAF protection"
  echo ""

  # Database public accessibility
  if [ -f "$OUTDIR/rds.csv" ]; then
    rds_public=$(awk -F',' 'NR>1 && $4 ~ /true/ {count++} END {print count+0}' "$OUTDIR/rds.csv" 2>/dev/null || echo "0")
    [ "$rds_public" -gt 0 ] && echo "  ⚠️  $rds_public RDS instances with PubliclyAccessible=true"
  fi

  if [ -f "$OUTDIR/redshift.csv" ]; then
    redshift_public=$(awk -F',' 'NR>1 && $4 ~ /true/ {count++} END {print count+0}' "$OUTDIR/redshift.csv" 2>/dev/null || echo "0")
    [ "$redshift_public" -gt 0 ] && echo "  ⚠️  $redshift_public Redshift clusters with PubliclyAccessible=true"
  fi

  # Lambda without auth
  if [ -f "$OUTDIR/lambda-urls.csv" ]; then
    lambda_noauth=$(grep -c 'NONE' "$OUTDIR/lambda-urls.csv" 2>/dev/null || echo "0")
    [ "$lambda_noauth" -gt 0 ] && echo "  ⚠️  $lambda_noauth Lambda URLs without authentication"
  fi

  # S3 public access
  if [ -f "$OUTDIR/s3.csv" ]; then
    s3_public=$(awk -F',' 'NR>1 && ($4 ~ /Yes/ || $5 ~ /Yes/ || $8 ~ /Yes/) {count++} END {print count+0}' "$OUTDIR/s3.csv" 2>/dev/null || echo "0")
    [ "$s3_public" -gt 0 ] && echo "  ⚠️  $s3_public S3 buckets with public access"
  fi


} | tee "$OUTDIR/SUMMARY.txt"

ok "✓ Enhanced attack surface enumeration complete!"
ok "✓ Results saved to: $OUTDIR"
echo ""
info "Output files generated:"
info "  • $OUTDIR/SUMMARY.txt - Full scan summary"
info "  • $OUTDIR/*.csv - Individual service inventories"
info "  • $OUTDIR/raw/ - Raw JSON responses from AWS APIs"
echo ""
